...
## 18. Features ponderadas por recencia (ronda 2026-09-02)

| Decisión | Estado sugerido | Detalle |
|---|---|---|
| Esquema de ponderación: decaimiento por posición (rank), `peso = 1/log2(rank+2)` (co-diseñado con el usuario) en vez de decaimiento exponencial por días o ventana dura de últimas N interacciones | ✅ **co-diseñado con el usuario** | Mismo descuento que ya usa `ndcg_at_k` -- no introduce una escala de tiempo (días/vida media) nueva que habría que barrer/validar aparte, a diferencia de un decaimiento exponencial. |
| 4 features nuevas (`n_libros_autor_leidos_reciente`/`n_libros_editorial_leidos_reciente`/`score_coleido_reciente`/`sim_resumen_historial_reciente`, 35→39), variantes recencia-ponderadas de señales ya existentes, sin tocar las 35 anteriores | ✅ **confirmado con el test pareado -- la señal más fuerte del proyecto con este método** | Test pareado (`scripts/comparar_features_pareado.py`, `n_por_fuente=75` -- ver fila de memoria más abajo): diferencia +0.007802 (+6.6%), **5.96σ**, bootstrap 95% CI [+0.005042, +0.010340] (no incluye 0), P(mejora)=100%. Muy por encima del umbral de 2σ y sin el patrón límite de género macro/editorial/señales cruzadas. CV 3 seeds (mismo `n_por_fuente`): ranker 0.127685±0.001509 vs ALS solo 0.094406±0.001359 (este último idéntico al histórico, confirma que el split sigue intacto). `feature_importances_` ubica a `n_libros_autor_leidos_reciente` entre las 2-3 features más importantes del set completo en los 3 seeds, por delante de su versión sin ponderar. **CONFIRMADO EN KAGGLE (junto con el ítem 19 -- ver esa sección)**: 0.06149, +16.9% sobre el récord anterior (0.05262) -- no se puede aislar el efecto individual de este ítem solo, porque se confirmó en la misma submission que el refit de etapa 1. Ver `bitacora.md`, sección "Ítem 1 de los pendientes". |
| Medir con `n_por_fuente=75` en vez del default de producción (150) | 🔄 **adaptación forzada por un problema de memoria de esta sesión puntual, no una decisión de diseño** | `generar_candidatos_con_features` empezó a fallar con `ArrayMemoryError` (RAM de sobra en la máquina -- fragmentación, no falta de memoria real) al construir el DataFrame final de ~5.5-6.2M filas con las 4 columnas nuevas. Se corrigieron dos causas reales (acumulación por fila en vez de por columna, mezcla de dtypes en vez de un solo bloque `float32`) pero `n_por_fuente=150` siguió fallando de forma intermitente en esta sesión -- no se identificó una causa raíz adicional en el tiempo disponible. Los números de esta ronda no son comparables en términos absolutos contra los históricos a `n_por_fuente=150`, pero la comparación pareada (39 vs 35 features, mismo `n_por_fuente` en ambos lados) sigue siendo válida. Pendiente: reproducir a `n_por_fuente=150` fuera de este entorno. |

## 19. Refit de etapa 1 sobre todos los datos para los candidatos finales (ronda 2026-09-02)

| Decisión | Estado sugerido | Detalle |
|---|---|---|
| `preparar_pipeline(..., refit_para_test=False)` -- con `True`, refitea ALS/popularidad/género/features auxiliares sobre `train_candidatos_full` (en vez de reusar el fit de `train_candidatos`) para generar los candidatos de test/`ndcg_als`, sin tocar el entrenamiento del ranker en sí | ✅ **confirmado con CV de 3 seeds -- la mejora más grande y menos ambigua de la sesión** | CV 3 seeds (`n_por_fuente=75`, misma limitación de memoria que la sección 18): sin refit 0.127685±0.001509 vs con refit 0.143336±0.002896 -- diferencia +0.014134/+0.016929/+0.015892 por seed, **positivo en los 3 seeds** y muy por encima del desvío entre seeds de cualquiera de las dos configs. El ALS-solo con refit (0.101473 en seed=42) coincide EXACTO con el histórico "ALS sobre split corregido" ya logueado -- confirma que la implementación está bien (fitear sobre `train_candidatos_full` y evaluar sobre `test_final` es exactamente ese escenario histórico). Ver `bitacora.md`, sección "Ítem 2 de los pendientes". |
| `submit.py::_recomendaciones_ranker` actualizado -- sigue entrenando el ranker sobre señales de `train_candidatos` (sin cambios, evita leakage de etiqueta), pero refitea sobre `interacciones` completo antes de generar `candidatos_finales` | ✅ **CONFIRMADO EN KAGGLE -- nuevo récord del proyecto** | Mismo patrón que `refit_para_test=True` en la evaluación local. **0.06149 en Kaggle, +16.9% sobre el récord anterior** (0.05262, +0.00887 absoluto) -- probablemente la mejora de una sola ronda más grande de todo el proyecto (por encima del +15.3% de autor/año/género/recencia). Confirmado en la MISMA submission que las features de recencia (sección 18) -- no se puede aislar el efecto individual de cada uno en Kaggle real con un solo dato, solo localmente por separado (CV +12.3% para el refit solo, test pareado 5.96σ para recencia sola). |

## 20. NDCG ponderado por la actividad real de `ejemplo.csv` (ronda 2026-09-02)

| Decisión | Estado sugerido | Detalle |
|---|---|---|
| `evaluation.pesos_por_actividad`/`evaluar_ndcg_ponderado_por_actividad` (nuevas, `BINS_ACTIVIDAD_DEFAULT` de 7 buckets) -- métrica REPORTADA, no cambia ningún criterio de decisión existente | ✅ **implementado y co-diseñado con el usuario** | Ya se había investigado a fondo que reponderar por actividad no cambia el signo de ninguna comparación (`bitacora.md`, "Investigando el sesgo sistemático") -- el usuario decidió cerrar este pendiente igual, como diagnóstico permanente en vez de análisis ad-hoc repetido cada ronda. `scripts/evaluate_ranker.py` la reporta al lado del NDCG sin ponderar; `preparar_pipeline`/`evaluar_con_params` ahora exponen `recs_als`/`recs_ranker` en sus dicts de salida para poder recalcular este diagnóstico sin reconstruir el ranking. |
| Verificación sin correr el pipeline completo (4 intentos de `scripts/evaluate_ranker.py` terminaron `killed` por la infraestructura de la sesión) | 🔄 **cableado verificado contra datos reales, número final pendiente** | Se verificó `pesos_por_actividad` directamente contra `load_interacciones()` + `data/raw/ejemplo.csv`: 46.9% de los usuarios de `ejemplo.csv` cae en el bucket 100+, coincide con el 47% ya medido en una sesión anterior -- confirma que el cruce está bien hecho. Un smoke test con recomendaciones perfectas dio NDCG ponderado = 1.0. Pendiente: correr `scripts/evaluate_ranker.py` completo fuera de este entorno para el número real de la config actual. Ver `bitacora.md`, sección "Ítem 3 de los pendientes". |

## 21. Particionar la generación de candidatos por lotes de usuarios (ronda 2026-09-02)

| Decisión | Estado sugerido | Detalle |
|---|---|---|
| `armar_dataset_entrenamiento_por_lotes`/`generar_candidatos_con_features_por_lotes`/`recomendar_por_usuario_por_lotes` (nuevas, `TAMANO_LOTE_USUARIOS=1000`) -- procesan usuarios en lotes en vez de materializar la unión de candidatos de toda la población de una sola vez | ✅ **CONFIRMADO EN KAGGLE -- comportamiento idéntico, memoria resuelta** | Motivado por el usuario, que notó que el pipeline consumía 32GB+ de RAM para un dataset chico (461K interacciones). Reusan `generar_candidatos_con_features`/`armar_dataset_entrenamiento`/`recomendar_por_usuario` tal cual -- solo cambia cuándo se arma y libera cada pedazo de memoria, no qué se calcula. Equivalencia confirmada con tests fila por fila (`tests/test_ranker.py`) y, de punta a punta, con una submission real: `ranker_20260902-204041_particionado.csv` (`n_por_fuente=150`, el default de producción que venía fallando con `ArrayMemoryError`) dio **0.06149 en Kaggle, idéntico** a la submission anterior (`n_por_fuente=75`, código sin particionar) -- mismo modelo, mismo resultado, memoria resuelta. `preparar_pipeline_cacheado` se simplificó de paso (ya no necesita `_comprimir_para_cache`/`_descomprimir_de_cache`, la compresión a `category` de `id_lector`/`id_libro` quedó como parte normal de `generar_candidatos_con_features_por_lotes`). Ver `bitacora.md`, sección "Particionar la generación de candidatos por lotes de usuarios". |

## 22. 7ª fuente de candidatos (intento): similitud usuario-usuario podada a k vecinos (ronda 2026-09-03)
...
## 25. Presupuesto propio para las fuentes de resumen y co-lectura (ronda 2026-09-07)
