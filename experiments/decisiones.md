...
| Hiperparámetros `factors=256, regularization=0.128, alpha=4.718, iterations=20` | v2 | ❌ **confirmado peor en Kaggle real** | Encontrados con optuna (30 trials) sobre un único split local: mejoraban el NDCG local +11.5%, pero al confirmar en Kaggle dieron **0.03341, peor que el 0.03864** del config anterior (`factors=128, regularization=0.1`, rating crudo). Sobreajuste al split fijo usado en el sweep — ver `bitacora.md`, sección "Regresión en Kaggle". Se decidió dejarlo así por ahora (no revertir) para seguir explorando el ranker de dos etapas, pero **no es el mejor confirmado en Kaggle** — pendiente de resolver. |
