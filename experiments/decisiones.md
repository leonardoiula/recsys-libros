...
| `submit.py::_recomendaciones_ranker` actualizado -- sigue entrenando el ranker sobre señales de `train_candidatos` (sin cambios, evita leakage de etiqueta), pero refitea sobre `interacciones` completo antes de generar `candidatos_finales` | ✅ **implementado, pendiente de confirmar en Kaggle** | Mismo patrón que `refit_para_test=True` en la evaluación local. Como este cambio y el de features de recencia (sección 18) ya conviven en el mismo código, una submission ahora confirmaría el efecto COMBINADO de ambos, no cada uno aislado -- si se quiere aislar el efecto individual de cada uno hace falta una submission por config (costo: presupuesto de submissions de Kaggle). |
