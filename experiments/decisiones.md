...
| Hiperparámetros `factors=256, regularization=0.128, alpha=4.718, iterations=20` | v2 | 🔄 revisar — **posible margen sin explotar** | Encontrados con optuna (30 trials) sobre el split corregido, reemplazan a `factors=128, regularization=0.1, alpha=1.0`. `factors` y `alpha` quedaron cerca del borde superior del rango buscado — una segunda pasada con rangos más amplios podría mejorar esto todavía más. No se hizo. |
...
| Combinar ALS + género/franja como features de un ranker (LightGBM) en vez de un solo modelo base | v2 | 🔄 sigue pendiente | Sigue siendo la idea "próximo candidato" de la bitácora — no se probó todavía. Es una idea distinta al ruteo por umbral (que sí se probó y se descartó, ver fila de arriba): acá ambos modelos aportarían señal simultánea a un tercer modelo, no uno reemplazando al otro por usuario. |
